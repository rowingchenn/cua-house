...
from cua_house.env_server.models import BatchCreateRequest, TaskRequirement, TaskState, utcnow
from cua_house.env_server.port_pool import PortPool
from cua_house.env_server.runtime import HostRuntimeConfig, ImageSpec
from cua_house.env_server.scheduler import EnvScheduler
...
class FakeRuntime:
    ...
    def prepare_slot(
        ...
        *,
        ...
        handle.container_name = f"agenthle-env-{slot_id}"
...
def make_scheduler() -> EnvScheduler:
    host = HostRuntimeConfig(
        ...
        runtime_root=Path("/tmp/agenthle-env-tests"),
...
def test_scheduler_defaults_and_grouping() -> None:
    async def scenario() -> None:
        ...
        batch = await scheduler.submit_batch(
            BatchCreateRequest(
                tasks=[
                    TaskRequirement(task_id="task-a", task_path="tasks/a", image_key="cpu-free"),
                    TaskRequirement(task_id="task-b", task_path="tasks/b", image_key="cpu-free"),
                ]
            )
        )
...
def test_lease_complete_releases_slot() -> None:
    async def scenario() -> None:
        ...
        await scheduler.start()
        await scheduler.submit_batch(
            BatchCreateRequest(tasks=[TaskRequirement(task_id="task-a", task_path="tasks/a", image_key="cpu-free")])
        )
        await wait_for_assignment(scheduler, "task-a")
...
def test_scheduler_starts_multiple_small_tasks_concurrently() -> None:
    async def scenario() -> None:
        ...
        await scheduler.submit_batch(
            BatchCreateRequest(
                tasks=[
                    TaskRequirement(task_id="task-a", task_path="tasks/a", image_key="cpu-free", cpu_cores=2, memory_gb=8),
                    TaskRequirement(task_id="task-b", task_path="tasks/b", image_key="cpu-free", cpu_cores=2, memory_gb=8),
                    TaskRequirement(task_id="task-c", task_path="tasks/c", image_key="cpu-free", cpu_cores=2, memory_gb=8),
                ]
            )
        )
        await asyncio.sleep(0.05)
...
def test_runtime_cleanup_orphaned_state(tmp_path: Path) -> None:
    ...
    from cua_house.env_server.runtime import DockerQemuRuntime
    ...
    assert calls[0] == ["docker", "ps", "-aq", "--filter", "name=agenthle-env-"]
...
def test_api_requires_bearer_token(monkeypatch) -> None:
    monkeypatch.setenv("AGENTHLE_TOKEN", "secret-token")
    from cua_house.env_server.api import create_app

    config_dir = Path(__file__).resolve().parents[1] / "src" / "cua_house" / "env_server" / "configs"

    app = create_app(
        host_config_path=config_dir / "agenthle_env_server.yaml",
        image_catalog_path=config_dir / "agenthle_env_images.yaml",
    )

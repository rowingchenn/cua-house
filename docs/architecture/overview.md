...
|-----------|------|----------------|
| **api** | `api/` | FastAPI app factory, route handlers, bearer auth, reverse proxy |
| **scheduler** | `scheduler/` | In-memory state machine, dispatch loop, lease reaper, batch heartbeat |
| **runtimes** | `runtimes/` | `RuntimeBackend` protocol, `DockerQemuRuntime`, `GCPVMRuntime` |
...
| **config** | `config/` | YAML config loader for `HostRuntimeConfig` and `ImageSpec` catalog |
...
- Multi-node scheduling (distribute across multiple KVM hosts)

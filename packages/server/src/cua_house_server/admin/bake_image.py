...
It allocates a temporary VM through cua-house-server, installs pinned
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--env-server-url", default=os.environ.get("CUA_HOUSE_SERVER_URL", ""))
    parser.add_argument("--env-server-token", default=os.environ.get("CUA_HOUSE_TOKEN"))
...
def freeze_overlay(
    *,
    ...
    ssh_script = f"""
set -euo pipefail
sudo systemctl stop cua-house-server
...
sudo systemctl is-active cua-house-server
""".strip()
...
def main() -> int:
    ...
    base_url = require_arg(args.env_server_url, "CUA_HOUSE_SERVER_URL")
